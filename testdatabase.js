...
let DataStudentFolder = path.join(__dirname, '/DataStudent');
let SumaryPointFolder = path.join(__dirname, '/SumaryPoint');
let DataFolder = path.join(__dirname, '/Data');
let PublicFolder = path.join(__dirname, '/public');
let TempFolderExecute = path.join(__dirname, '/Data/execute_exe');
let CreateTestcaseFolder = path.join(__dirname, '/CreateTestcase');
let RunTestcaseFolder = path.join(__dirname, '/RunTestcase');
let CheckPointFolder = path.join(__dirname, '/CheckPoint');
let LecturerFolder = path.join(__dirname, '/Lecturer');
let TestcaseFolder = path.join(__dirname, '/Testcase');
let StudentOutputFile = path.join(__dirname, 'output.txt');
let StudentErrorFile = 'error.txt';
let AccountFile = path.join(DataStudentFolder, '/Account.csv');
let XmlFile = 'project.xml';
let TopicFile = path.join(LecturerFolder, '/Topic.csv');
let LibStdFile = 'libstdc++-6.dll';
const  fs = require('fs');
const linereader = require('line-reader');
const copyandmove = require('./copymove.js');
const childprocess = require('child_process');
const extract = require('extract-zip');
const xmlcompile = require('./xml_compile.js');
...
function getTimesSubmitOfStudent(username, numtopic, onsuccess) {
  let fileData = path.join(SumaryPointFolder, 'Topic' + numtopic + '.csv');
...
module.exports = {
  ...
  getCellInData: function (id, source, collum, onsuccess) {
    linereader.eachLine(source, function (line, last) {
      ...
      if (separate[0] == id) {
        onsuccess(separate[Number(collum)]);
      }
    });
  },
  ...
  editNameAfterUnzip: function (temp, source, onsuccess) {
    fs.readdir(temp, function (err, files) {
      copyandmove.moveFile(path.join(temp, files[0]), source, onsuccess);
    });
  },
    ...
    db.getCellInData(numtopic, TopicFile, '2', function (filetestcase) {
      ...
      linereader.eachLine(TestcaseFolder + '/' + filetestcase + '.txt', function (line, last) {
        ...
        for (let i = 0; i < separate1.length; ++i) {
          ...
          arrnum.push(separate2[0]);
          arrweight.push(separate2[1]);
        }
      ...
      db.getCellInData(nowtopic, TopicFile, '3', function (filecreatetestcase) {
        for (let i = 0; i < arrnum.length; ++i) {
          ...
          fs.writeFile(numtestcasefile, arrnum[i], function (err) {
            if (err) return console.log(err);
            childprocess.exec(path.join(CreateTestcaseFolder, filecreatetestcase + '.exe' + ' <numtestcase' + (i + 1) + '.txt> ' + PrefixOfInputFile + (i + 1) + '.txt'), {
                cwd: builddir
              },
  ...
  runTestcase: function (id, numtopic, now_submit, arrnum, success) {
    ...
    db.getCellInData(numtopic, TopicFile, 4, function (nameruntestcasefile) {
      RunTestcaseFolder = path.join(__dirname, '/RunTestcase');
      let runtestcasefile = path.join(RunTestcaseFolder, nameruntestcasefile + '.exe');
      let folderbuild = path.join(DataFolder, id, 'topic' + numtopic, 'submit' + now_submit, 'build');
      for (let i = 0; i < arrnum.length; ++i) {
        childprocess.exec(runtestcasefile + ' <' + PrefixOfInputFile + (i + 1) + '.txt> ' + PrefixOfResultFile + (i + 1) + '.txt', {
          cwd: folderbuild
        }, function (err, stdout, stderr) {
          if (err) return console.log(err);
          else {
            if (i == arrnum.length - 1) success();
          }
        });
      }
    });
  ...
  unzipFileSubmit: function (id, numtopic, times_submit, success) {
    ...
    let filetopic = path.join(SumaryPointFolder, 'Topic' + numtopic + '.csv');
    let newpath = path.join(DataFolder, id, 'topic' + numtopic, 'submit' + now_submit + '.zip');
    let tempfolder = path.join(DataFolder, id,'topic' + numtopic, 'temp');
    ...
    extract(newpath, {
      ...
      if (err) return console.log(err);
        ...
        let newfolder = path.join(DataFolder, id, 'topic' + numtopic, 'submit' + now_submit);
        db.editNameAfterUnzip(tempfolder, newfolder, function () {
          let NowTopicFile = path.join(SumaryPointFolder, 'Topic' + numtopic + '.csv');
              ...
              db.runTestcase(id, numtopic, now_submit, arrnum, function () {
                db.processProject(id, numtopic, arrnum, arrweight, function (point) {
                  db.editCellInData(id, filetopic, 2, point, function () {
                    ...
                    db.AppendFileHistorySubmit(id, numtopic, infoTimesSubmit, success);
                  });
                });
              });
            });
    ...
    db.getCellInData(numtopic, TopicFile, 5, function (filecheckpoint) {
      db.createXmlFileFromFolder(id, numtopic, function (times_submit) {
        xmlcompile.readXmlAndCompile(id, numtopic, times_submit, function (error, success) {
          if (error) db.noteError(id, numtopic, times_submit, 'Error In Compile !', function () {
            result(point);
          });
          else {
